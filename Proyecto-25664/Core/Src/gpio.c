...
Descripción:
Módulo de control de entradas y salidas digitales (GPIO) para el microcontrolador
STM32F4. Este archivo implementa funciones para la configuración de pines como
entrada, salida o modo alternativo, así como la lectura y escritura de estados
digitales. Incluye utilidades para el manejo de múltiples pines y la generación
de secuencias de activación en LEDs.
*/
...
// =========== Función para establecer Iput un rango de pines del mismo  puerto ===========
...
void digitalWrite(GPIO_TypeDef *port,int pin, int state){
	if(state == 1){port -> BSRR = (1U << pin);} else{port -> BSRR = (1U << (pin + 16));}
}

// =========== Aquí recibe la secuencia y la ejectuta ===========
